...
const BLACK = "#111111";
const WHITE = "#FFFFFF";
const FONT = "font-family=\"'DIN Alternate','Helvetica Neue',Arial,sans-serif\" font-weight=\"700\"";

const manifest = [];
function emit(kind, id, alt, svg) {
  ...
  manifest.push({ id, file, kind, alt, credit: kind === "sign" ? "Verkehrszeichen nach StVO (amtliches Werk, § 5 UrhG); Vektorzeichnung FahrPilot" : "Grafik FahrPilot (eigene Darstellung, schematisch)" });
}
...
// Zeichenformen
// ---------------------------------------------------------------------------------------------
const svgOpen = (w, h, title) => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" role="img" aria-label="${esc(title)}"><title>${esc(title)}</title>`;
const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/"/g, "&quot;");
const pict = (inner, x, y, size) => `<svg x="${x}" y="${y}" width="${size}" height="${size}" viewBox="0 0 100 100">${inner}</svg>`;

function triangleUp(inner, title, pictSize = 52) {
...
function circleRed(inner, title, pictSize = 68, extra = "") {
...
function circleBlue(inner, title, pictSize = 76) {
...
function rectBlue(w, h, inner, title) {
...
function rectWhite(w, h, inner, title) {
...
const text = (x, y, size, str, fill = BLACK, anchor = "middle", extra = "") => `<text x="${x}" y="${y}" font-size="${size}" ${FONT} fill="${fill}" text-anchor="${anchor}" ${extra}>${esc(str)}</text>`;

// Anhalt: Kreis mit roten Balken (283/286)
function haltverbot(bars, title) {
...
function zusatz(w, h, inner, title) {
...
const SIGNS = {
  ...
  "209-10": { alt: "Zeichen 209 Vorgeschriebene Fahrtrichtung rechts: blauer Kreis mit weißem Pfeil nach rechts", svg: () => circleBlue(`<g transform="rotate(90 50 50)">${P.arrowUpWhite}</g>`, "Zeichen 209 Rechts") },
...
for (const [id, s] of Object.entries(SIGNS)) emit("sign", id, s.alt, s.svg());
...
scene("polizist-seitlich", "Ein Polizeibeamter steht mit seitlich ausgestreckten Armen quer zu Ihrer Fahrtrichtung: Sie sehen ihn von der Seite mit ausgestrecktem Arm zu Ihnen, das bedeutet Halt.", 600, 400,
...
console.log(`${manifest.length} Medien erzeugt (${manifest.filter((m) => m.kind === "sign").length} Zeichen, ${manifest.filter((m) => m.kind === "scene").length} Grafiken)`);
