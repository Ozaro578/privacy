...
export function StudentShell({ children, schoolName, legalBasisDate, current, selfStudy = false }: { children: ReactNode; schoolName: string; legalBasisDate: string | null; current: string; selfStudy?: boolean }) {
  const tabs = selfStudy ? TABS.filter((t) => t.href !== "/fahren" && t.href !== "/finanzen") : TABS;
  return (
    <div className="mx-auto flex min-h-dvh max-w-5xl flex-col md:flex-row">
      <nav aria-label="Hauptnavigation" className="hidden w-56 shrink-0 flex-col border-r border-ink-100 p-4 md:flex">
        ...
        {!selfStudy && <Link href="/theorie" className={`mb-1 rounded-xl px-3 py-2 ${current.startsWith("/theorie") ? "bg-brand-50 font-semibold text-brand-700" : "text-ink-700 hover:bg-ink-100"}`}>Theorieunterricht</Link>}
