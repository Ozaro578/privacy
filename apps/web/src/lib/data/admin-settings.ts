...
export async function getSettings() {
  ...
  const [{ data: locations }, { data: policies }, { count: memberCount }] = await Promise.all([
  ...
  return { ctx, settings: schoolSettings(ctx.school.settings), locations: locations ?? [], policies: policies ?? [], memberCount: memberCount ?? 0, registrationUrl, qrSvg };
}
