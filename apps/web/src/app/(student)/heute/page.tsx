  ...
  return (
    <div className="space-y-5">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">{greeting()}, {ctx.student.first_name} 👋</h1>
          <p className="text-sm text-ink-700">{ctx.licenseInfo.name} · {ctx.license.transmission === "automatic" ? "Automatik" : "Schaltung"}</p>
        </div>
      ...
      {ctx.rules.examTheory?.needsVerification && <Alert tone="warning" title="Regelwerte in Prüfung">Die Prüfungsregeln für {ctx.licenseInfo.name} sind hinterlegt, aber noch nicht fachlich freigegeben. Simulationen sind vorläufig.</Alert>}
